...
A repository AGENTS.md or CLAUDE.md, a skill, or a direct request from me wins
over this file. Some formats have a fixed shape. Keep that shape, and write STE
inside it.
...
After you create the worktree, do all later writes in the worktree path that
the skill reports. If your tool can move the session into a directory, move
it into that path.
...
I use a `ctx` system. It connects a work context (a ticket, a project, or a
topic) to a local directory.

- `~/.config/active-ctx` holds the name of the current context, for example
  `GTI-197` or `chezmoi-refactor`.
- `~/ctx/<name>/` is the directory of the context. It can hold HTML research
  artifacts, notes, data, or other files.
- `$CTX_DIR` is an environment variable in each shell. It points to the
  directory of the active context.

Do not read, scan, or summarize the active context when a session starts.
Read it only when I ask, for example "look at my research" or "check my
context". Use `$CTX_DIR` directly. Do not read `~/.config/active-ctx` or list
`~/ctx/*`.

When you save an HTML artifact for me, write it to `$CTX_DIR`. If `$CTX_DIR`
is not set, write it to `~/ctx/<name>/`.

These shell functions control the context:

- `ctx <name>` switches to a context. It makes `~/ctx/<name>/` if necessary,
  and it sets `$CTX_DIR`.
